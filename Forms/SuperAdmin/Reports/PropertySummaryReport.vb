...
Public Class PropertySummaryReport
    Private Sub PropertySummaryReport_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ' Load property data automatically
        LoadPropertyData()
    End Sub
            ...
            Try
                ...
                If conn IsNot Nothing AndAlso conn.State = ConnectionState.Open Then
                    Try
                        ...
                        Using cmd As New MySqlCommand("SELECT DISTINCT category FROM properties WHERE category IS NOT NULL AND category <> '' ORDER BY category", conn)
    ...
    Private Sub LoadPropertyData()
        Try
            ' Build query to get all properties with full details including assigned user info
            Dim query As String = "SELECT 
                ...
                p.description,
                p.unitOfMeasure,
                COALESCE(p.acquisitionCost, 0) as acquisitionCost,
                COALESCE(p.totalCost, 0) as totalCost,
                COALESCE(p.sourceOfFunds, '') as sourceOfFunds,
                CONCAT(COALESCE(u.firstName, ''), ' ', COALESCE(u.lastName, '')) as assignedTo,
                COALESCE(d.departmentName, '') as departmentName,
                COALESCE(p.location, '') as location,
                COALESCE(p.condition, '') as propertyCondition,
                COALESCE(p.status, '') as propertyStatus,
                DATE_FORMAT(p.acquisitionDate, '%Y-%m-%d') as acquisitionDate
            FROM properties p
                        ...
                        Using adapter As New MySqlDataAdapter(cmd)
                            ...
                            For Each row As DataRow In dt.Rows
                                DataGridView1.Rows.Add(
                                    ...
                                    If(row.IsNull("unitOfMeasure"), "", row("unitOfMeasure")),
                                    If(row.IsNull("totalCost"), "0.00", FormatNumber(row("totalCost"), 2)),
                                    If(String.IsNullOrWhiteSpace(row("assignedTo").ToString()), "", row("assignedTo").ToString().Trim()),
                                    If(row.IsNull("location"), "", row("location")),
                                    If(row.IsNull("propertyCondition"), "", row("propertyCondition")),
    ...
    Private Sub ExportToCSV(filePath As String)
        Using writer As New StreamWriter(filePath, False, New UTF8Encoding(True))
            ...
            writer.WriteLine()
            writer.WriteLine("PROPERTY MANAGEMENT SUMMARY")
            writer.WriteLine()
            ...
            For Each row As DataGridViewRow In DataGridView1.Rows
                If Not row.IsNewRow Then
                    Dim values As New List(Of String) From {
                        ...
                        EscapeCSV(row.Cells(7).Value?.ToString()),
                        EscapeCSV(row.Cells(8).Value?.ToString())
                    }
