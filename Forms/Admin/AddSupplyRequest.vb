...
Public Class AddSupplyRequest
    ...
    Private currentDepartmentId As Integer?
    ...
    ' Constructor with pre-filled data (called from SupplyInventory)
    ' Parameters match what SupplyInventory passes: itemName, description, category, availableQuantity, location, unitOfMeasure, requesterName, position, department, currentDate
    Public Sub New(itemName As String, Optional description As String = "", Optional category As String = "", Optional availableQuantity As String = "1", Optional location As String = "", Optional unitOfMeasure As String = "", Optional requesterName As String = "", Optional position As String = "", Optional department As String = "", Optional currentDate As String = "")
        InitializeComponent()
        InitializeForm()
        ...
        ' Set quantity to 1 by default (staff can change it)
        numQuantity.Value = 1
    ...
    Private Sub txtItemName_TextChanged(sender As Object, e As EventArgs)
        ' Auto-fill unit of measure based on existing supply with the same item name
        Try
    ...
    Private Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
        ...
        ' Get department ID
        Dim departmentId As Integer? = currentDepartmentId
        ...
        Try
            ' Prepare data for supplies_requests table
            ...
            If conn IsNot Nothing AndAlso modDB.SafeOpenConnection(conn) Then
                Try
                    Dim query As String = "INSERT INTO supplies_requests (requesterName, position, departmentId, dateOfRequest, " &
                                         ...
                                         "VALUES (@requesterName, @position, @departmentId, @dateOfRequest, " &
                                         "@itemName, @description, @quantityRequested, @unit, @purpose, 'Pending', NOW(), NOW())"
                    
                    Using cmd As New MySqlCommand(query, conn)
                        cmd.Parameters.AddWithValue("@requesterName", requesterName)
                        cmd.Parameters.AddWithValue("@position", If(position, DBNull.Value))
                        cmd.Parameters.AddWithValue("@departmentId", If(departmentId, DBNull.Value))
                        ...
                        cmd.Parameters.AddWithValue("@unit", If(unit, DBNull.Value))
                        cmd.Parameters.AddWithValue("@purpose", purpose)
                        
                        Dim rowsAffected As Integer = cmd.ExecuteNonQuery()
                        
                        If rowsAffected > 0 Then
                            MessageBox.Show("Supply request submitted successfully!" & Environment.NewLine & "Your request is pending approval.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            ClearForm()
                            NavigateBackToList()
                        Else
        ...
        Catch ex As Exception
            ...
            System.Diagnostics.Debug.WriteLine("AddSupplyRequest Error: " & ex.Message & Environment.NewLine & ex.StackTrace)
        End Try
    End Sub
    ...
    Private Sub NavigateBackToList()
        ' Navigate back to Supply Inventory or parent dashboard
        ...
        Else
            ...
            If adminDashboard IsNot Nothing Then
                adminDashboard.LoadUserControl(New UC_SupplyRequestManagement())
            Else
