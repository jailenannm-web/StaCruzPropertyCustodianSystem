...
Public Class DatabaseConnection
    ...
    Public Shared Function GetPropertyInventoryReport(Optional groupByCategory As Boolean = False,
        ...
        Try
            ...
            Else
                query.Append("SELECT p.property_id, p.item_name, p.category, p.status, p.location, ")
            ...
            query.Append("LEFT JOIN users u ON p.assigned_to = u.user_id WHERE 1=1 ")
            ...
            Else
                query.Append(" ORDER BY p.category, p.item_name")
            End If
    ...
    Public Shared Function GetSupplyInventoryReport(Optional category As String = "",
        ...
        Try
            ...
            Dim query As New StringBuilder()
            query.Append("SELECT s.supply_id, s.item_name, s.category, s.quantity, ")
            query.Append("s.unit_cost, s.total_cost, s.stock_status, s.location ")
            query.Append("FROM supplies s WHERE 1=1 ")

            If Not String.IsNullOrEmpty(category) Then query.Append(" AND s.category = @category ")
            If Not String.IsNullOrEmpty(status) Then query.Append(" AND s.stock_status = @status ")
            If includeLowStockOnly Then query.Append(" AND s.quantity <= 10 ")

            query.Append(" ORDER BY s.category, s.item_name")
    ...
    Public Shared Function GetRequestTransactionsReport(Optional status As String = "",
        ...
        Try
            ...
            query.Append("LEFT JOIN users sa ON pr.user_id = sa.user_id ")
    ...
    Public Shared Function GetCustodianAssignmentsReport(Optional custodianID As Integer? = Nothing,
        ...
        Try
            ...
            query.Append("d.department_name, p.item_name AS asset_name, p.category AS asset_category, 'Property' AS asset_type ")
            query.Append("FROM users u ")
            query.Append("INNER JOIN properties p ON u.user_id = p.assigned_to ")
            ...
            query.Append("SELECT NULL AS user_id, 'Unassigned' AS custodian_name, ")
            query.Append("'' AS department_name, s.item_name AS asset_name, s.category AS asset_category, 'Supply' AS asset_type ")
            query.Append("FROM supplies s WHERE 1=1 ")
    ...
    Public Shared Function GetRequisitionAndIssuanceReport(Optional dateFrom As Date? = Nothing,
        ...
        Try
            ...
            query.Append("d.department_name, COALESCE(p.item_name, sup.item_name) AS item_name ")
            query.Append("FROM property_requests pr ")
            query.Append("INNER JOIN users sa ON pr.user_id = sa.user_id ")
    ...
    Public Shared Function AuthenticateStaff(username As String,
        ...
        Try
            ...
            ' Check hardcoded staff credentials first
            Dim hardcodedStaffUsername As String = "test_staff"
            Dim hardcodedStaffPassword As String = "Staff@1234"
                ...
                Using cmd As New MySqlCommand(hardcodedQuery, conn)
                    ...
                    Using reader As MySqlDataReader = cmd.ExecuteReader()
                        If reader.Read() Then
                            ...
                            result("first_name") = If(IsDBNull(reader("first_name")), "", reader("first_name").ToString())
                            result("last_name") = If(IsDBNull(reader("last_name")), "", reader("last_name").ToString())
                            result("email") = If(IsDBNull(reader("email")), "", reader("email").ToString())
                            result("contact_number") = If(IsDBNull(reader("contact_number")), "", reader("contact_number").ToString())
                            ...
                            LogActivity(userID, "Staff", hardcodedStaffUsername, "LOGIN", "Authentication", "Staff successfully logged in (hardcoded)", ipAddress)
    ...
    Private Shared Function AuthenticateWithHardcodedCredentials(username As String,
        ...
        Const superAdminPassword As String = "SuperAdmin@2025"
        Const adminUsername As String = "admin"
        Const adminPassword As String = "Admin@2025"
        Const custodianUsername As String = "custodian"
        Const custodianPassword As String = "Custodian@2025"
        ...
        Try
            If normalizedUsername = superAdminUsername AndAlso password = superAdminPassword Then
                ...
                result = EnsureHardcodedAccount(superAdminUsername,
                                                superAdminPassword,
                                                "Super",
                                                "Administrator",
                                                ...
                                                "SuperAdmin")
                If result.Count > 0 Then
                    RecordAdminLogin(CInt(result("user_id")), "SuperAdmin", result("username"), ipAddress)
            ...
            If normalizedUsername = adminUsername AndAlso password = adminPassword Then
                ...
                result = EnsureHardcodedAccount(adminUsername,
                                                adminPassword,
                                                "System",
                                                "Administrator",
                                                "admin@stacruz.edu",
                                                "Admin")
                If result.Count > 0 Then
                    RecordAdminLogin(CInt(result("user_id")), "Admin", result("username"), ipAddress)
            ...
            If normalizedUsername = custodianUsername AndAlso password = custodianPassword Then
                ...
                result = EnsureHardcodedCustodianAccount(custodianUsername,
                                                         custodianPassword,
                                                         "Property",
                                                         "Custodian",
                                                         "custodian@stacruz.edu",
                                                         ipAddress)
                If result.Count > 0 Then
                    System.Diagnostics.Debug.WriteLine("[v0] AuthenticateWithHardcodedCredentials - Custodian login successful, user_id: " & result("user_id"))
                Else
    ...
    Private Shared Function EnsureHardcodedAccount(username As String,
        ...
        Try
            ...
            If account.Count = 0 Then
                Using insertCmd As New MySqlCommand("INSERT INTO users (first_name, last_name, email, username, password_encrypted, role, status, created_at) " &
            ...
            Else
                Using updateCmd As New MySqlCommand("UPDATE users SET first_name = @firstName, last_name = @lastName, email = @email, " &
                                                    "password_encrypted = @password, role = @userType, status = 'Active' WHERE user_id = @userID", conn)
                    ...
                    updateCmd.Parameters.AddWithValue("@userID", CInt(account("user_id")))
                    updateCmd.ExecuteNonQuery()
                End Using
            End If
    ...
    Private Shared Function FetchAdminAccountRecord(conn As MySqlConnection, username As String) As Dictionary(Of String, String)
        Dim account As New Dictionary(Of String, String)
        Dim query As String = "SELECT user_id, first_name, last_name, email, username, role AS user_type " &
                              "FROM users WHERE LOWER(username) = LOWER(@username) LIMIT 1"
        Using cmd As New MySqlCommand(query, conn)
            cmd.Parameters.AddWithValue("@username", username.Trim())
            Using reader As MySqlDataReader = cmd.ExecuteReader()
                If reader.Read() Then
                    account("user_id") = reader("user_id").ToString()
                    account("first_name") = reader("first_name").ToString()
                    account("last_name") = reader("last_name").ToString()
    ...
    Private Shared Function EnsureHardcodedCustodianAccount(username As String,
        ...
        Try
            ...
            Using checkCmd As New MySqlCommand("SELECT user_id FROM users WHERE LOWER(username) = LOWER(@username) AND role = 'Custodian' LIMIT 1", conn)
            ...
            Else
                ...
                Using updateCmd As New MySqlCommand("UPDATE users SET first_name = @firstName, last_name = @lastName, email = @email, " &
                                                    "password_encrypted = @password, status = 'Active', updated_at = NOW() WHERE user_id = @userID", conn)
                    updateCmd.Parameters.AddWithValue("@firstName", firstName)
                    updateCmd.Parameters.AddWithValue("@lastName", lastName)
                    updateCmd.Parameters.AddWithValue("@email", email)
                    updateCmd.Parameters.AddWithValue("@password", hashedPassword)
            ...
            Using fetchCmd As New MySqlCommand("SELECT user_id, first_name, last_name, email, username, role, status " &
                ...
                Using reader As MySqlDataReader = fetchCmd.ExecuteReader()
                    If reader.Read() Then
                        Dim userID As Integer = Convert.ToInt32(reader("user_id"))
                        account("staff_id") = userID.ToString()
                        ...
                        System.Diagnostics.Debug.WriteLine("[v0] EnsureHardcodedCustodianAccount - Account fetched successfully (ID: " & account("user_id") & ")")
                    Else
            ...
            If account.Count > 0 Then
                Dim userID As Integer = CInt(account("user_id"))
                Using updateCmd As New MySqlCommand("UPDATE users SET last_login = NOW() WHERE user_id = @userID", conn)
                    updateCmd.Parameters.AddWithValue("@userID", userID)
                    updateCmd.ExecuteNonQuery()
                End Using
    ...
    Public Shared Function ValidateAdminLogin(username As String, password As String, Optional ipAddress As String = "") As Dictionary(Of String, String)
            ...
            If password = superAdminPassword Then
                ...
                Try
                    ...
                    If conn IsNot Nothing AndAlso SafeOpenConnection(conn) Then
                        ...
                        Dim query As String = "SELECT user_id, first_name, last_name, email, username, role AS user_type, password_encrypted " &
                        ...
                        Using cmd As New MySqlCommand(query, conn)
                            ...
                            If hardcodedSuccess Then
                                ...
                                LogActivity(CInt(result("user_id")), "SuperAdmin", result("username"), "LOGIN", "Authentication", "SuperAdmin successfully logged in", ipAddress)
                            End If
                        End Using
                    End If
                Finally
                    ...
                    If conn IsNot Nothing AndAlso SafeOpenConnection(conn) Then
                        ...
                        Using cmd As New MySqlCommand(query, conn)
                            ...
                            If hardcodedAdminSuccess Then
                                ...
                                LogActivity(CInt(result("user_id")), "Admin", result("username"), "LOGIN", "Authentication", "Admin successfully logged in", ipAddress)
                            End If
                        End Using
                    End If
                Finally
        ...
        Try
            ...
            If result.Count > 0 Then
                Using updateCmd As New MySqlCommand("UPDATE users SET last_login = NOW() WHERE user_id = @userID", conn2)
                    updateCmd.Parameters.AddWithValue("@userID", result("user_id"))
                    updateCmd.ExecuteNonQuery()
                End Using
    ...
    Public Shared Function GetStaffDetails(username As String) As Dictionary(Of String, String)
        ...
        Try
            ...
            Dim query As String = "SELECT user_id, first_name, last_name, email, department_id FROM users WHERE username = @username AND role = 'Staff'"
            Using cmd As New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@username", username)
                Using reader As MySqlDataReader = cmd.ExecuteReader()
                    If reader.Read() Then
                        staffDetails("staff_id") = reader("user_id").ToString()
                        staffDetails("user_id") = reader("user_id").ToString()
                        staffDetails("first_name") = If(IsDBNull(reader("first_name")), "", reader("first_name").ToString())
                        staffDetails("last_name") = If(IsDBNull(reader("last_name")), "", reader("last_name").ToString())
                        staffDetails("email") = If(IsDBNull(reader("email")), "", reader("email").ToString())
                        staffDetails("department_id") = If(IsDBNull(reader("department_id")), "", reader("department_id").ToString())
                    End If
                End Using
            End Using
        Catch ex As Exception
    ...
    Public Shared Function GetStaffProfile(staffID As Integer) As Dictionary(Of String, Object)
        ...
        Try
            ...
            Dim query As String =
                "SELECT user_id, first_name, middle_name, last_name, suffix, position, department_id, " &
                "contact_number, email, username, date_assigned, employee_id, status, last_login, created_at " &
                "FROM users WHERE user_id = @staffID AND role = 'Staff'"

            Using cmd As New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@staffID", staffID)
                Using reader As MySqlDataReader = cmd.ExecuteReader()
                    If reader.Read() Then
                        profile("staff_id") = reader("user_id")
                        profile("user_id") = reader("user_id")
    ...
    Public Shared Function AddSupply(supplyID As String, supplyName As String, category As String,
        ...
        Try
            ...
            Dim query As String = "INSERT INTO supplies (" &
                                   "item_name, category, quantity, unit_cost, stock_status, location, " &
                                   "description, unit_of_measure, supplier, date_received, total_cost) " &
                                   "VALUES (@supplyName, @category, @stock, @unitCost, @status, @location, " &
                                   "@description, @uom, @supplierName, NOW(), @totalValue)"
    ...
    Public Shared Function GetSupplyDashboardStats() As Dictionary(Of String, Integer)
        ...
        Try
            ...
            ' Available supplies
            Dim availableQuery As String = "SELECT COUNT(*) FROM supplies WHERE stock_status = 'Available'"
            Using cmd As New MySqlCommand(availableQuery, conn)
                stats("available_supplies") = CInt(cmd.ExecuteScalar())
            End Using

            ' Low stock supplies
            Dim lowStockQuery As String = "SELECT COUNT(*) FROM supplies WHERE quantity <= 10 AND stock_status = 'Available'"
    ...
    Public Shared Function GetSupplyStatusCounts() As DataTable
        Dim query As String = "SELECT IFNULL(stock_status, 'unspecified') AS label, COUNT(*) AS total " &
                              "FROM supplies GROUP BY stock_status ORDER BY stock_status"
        Return ExecuteLookupDataTable(query)
    End Function
    ...
    Public Shared Function SubmitPropertyRequest(userID As Integer, itemName As String, purpose As String,
        ...
        Try
            ...
            If Not finalDeptID.HasValue AndAlso userID > 0 Then
                Try
                    Using userCmd As New MySqlCommand("SELECT department_id FROM users WHERE user_id = @userID LIMIT 1", conn)
    ...
    Public Shared Function StaffSubmitSupplyRequest(staffID As Integer,
        ...
        Try
            ...
            If Not finalDeptID.HasValue AndAlso staffID > 0 Then
                Try
                    Using userCmd As New MySqlCommand("SELECT department_id FROM users WHERE user_id = @staffID LIMIT 1", conn)
    ...
    Public Shared Function GetAllPropertyRequests() As DataTable
        ...
        Try
            ...
            Dim query As String = "SELECT " &
                                 ...
                                 "IF(u.suffix IS NOT NULL AND u.suffix != '', CONCAT(' ', u.suffix), ''))) AS requester_name, " &
                                 "IFNULL(d.department_name, 'N/A') AS department, " &
                                 "pr.date_of_request AS date_of_request, " &
                                 "pr.item_name AS item_name, " &
                                 "pr.quantity_requested AS quantity_requested, " &
    ...
    Public Shared Function GetStaffRequests(staffID As Integer,
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(requestTypeFilter) AndAlso requestTypeFilter.ToLower() = "property" Then
                ...
                query = "SELECT pr.request_id, 'property' AS request_type, pr.status, pr.date_of_request AS request_date, " &
                        "pr.approved_date AS approval_date, NULL AS release_date, NULL AS expected_return_date, " &
                        "NULL AS actual_returned_date, pr.quantity_requested AS quantity, " &
                        "NULL AS penalty, NULL AS condition_upon_return, pr.purpose AS remarks, " &
                        "pr.item_name, NULL AS serial_number, NULL AS property_location, NULL AS supply_location " &
                        "FROM property_requests pr " &
                        "WHERE pr.user_id = @staffID "
            ...
            ElseIf Not String.IsNullOrEmpty(requestTypeFilter) AndAlso requestTypeFilter.ToLower() = "supply" Then
                ...
                query = "SELECT sr.request_id, 'supply' AS request_type, sr.status, sr.date_of_request AS request_date, " &
                        "sr.approved_date AS approval_date, NULL AS release_date, NULL AS expected_return_date, " &
                        "NULL AS actual_returned_date, sr.quantity_requested AS quantity, " &
                        "NULL AS penalty, NULL AS condition_upon_return, sr.purpose AS remarks, " &
                        "sr.item_name, NULL AS serial_number, NULL AS property_location, NULL AS supply_location " &
            ...
            Else
                ' Show both - UNION ALL
                Dim propQuery As String = "SELECT pr.request_id, 'property' AS request_type, pr.status, pr.date_of_request AS request_date, " &
                        "pr.approved_date AS approval_date, NULL AS release_date, NULL AS expected_return_date, " &
                        "NULL AS actual_returned_date, pr.quantity_requested AS quantity, " &
                        "NULL AS penalty, NULL AS condition_upon_return, pr.purpose AS remarks, " &
                        "pr.item_name, NULL AS serial_number, NULL AS property_location, NULL AS supply_location " &
                ...
                Dim supplyQuery As String = "SELECT sr.request_id, 'supply' AS request_type, sr.status, sr.date_of_request AS request_date, " &
                        "sr.approved_date AS approval_date, NULL AS release_date, NULL AS expected_return_date, " &
                        "NULL AS actual_returned_date, sr.quantity_requested AS quantity, " &
                        "NULL AS penalty, NULL AS condition_upon_return, sr.purpose AS remarks, " &
                        "sr.item_name, NULL AS serial_number, NULL AS property_location, NULL AS supply_location " &
    ...
    Public Shared Function GetDetailedPropertyRequests(Optional statusFilter As String = "",
        ...
        Try
            ...
            query.Append("pr.approved_date, d.department_name, pr.remarks ")
            query.Append("FROM property_requests pr ")
            query.Append("LEFT JOIN departments d ON pr.department_id = d.department_id WHERE 1=1 ")
    ...
    Public Shared Function ApprovePropertyRequest(requestID As Integer, adminID As Integer, adminName As String,
        ...
        Try
            ...
            Using checkCmd As New MySqlCommand("SELECT status, item_name, quantity_requested FROM property_requests WHERE request_id = @requestID FOR UPDATE", conn, transaction)
            ...
            Using updateRequest As New MySqlCommand("UPDATE property_requests SET status = 'Approved', approved_by = @adminID, " &
                                                    "approved_date = NOW(), remarks = @remarks WHERE request_id = @requestID", conn, transaction)
    ...
    Public Shared Function RejectPropertyRequest(requestID As Integer, adminID As Integer, adminName As String,
        ...
        Try
            ...
            Dim query As String = "UPDATE property_requests SET status = 'Rejected', approved_by = @adminID, approved_date = NOW(), remarks = @remarks " &
    ...
    Public Shared Function GetStaffBorrowedItems(staffID As Integer,
        ...
        Try
            ...
            If Not includeHistory Then
                ' Only show approved/released items
                query = "SELECT pr.request_id, 'property' AS request_type, pr.status, pr.date_of_request AS request_date, " &
                        "NULL AS release_date, NULL AS expected_return_date, NULL AS actual_returned_date, " &
                        "pr.quantity_requested AS quantity, pr.item_name AS item_name, NULL AS serial_number, " &
                        "'Not yet returned' AS accountability_status " &
                        "FROM property_requests pr " &
                        "WHERE pr.user_id = @staffID AND pr.status IN ('Approved', 'approved', 'Released', 'released') " &
                        "UNION ALL " &
                        "SELECT sr.request_id, 'supply' AS request_type, sr.status, sr.date_of_request AS request_date, " &
                        "NULL AS release_date, NULL AS expected_return_date, NULL AS actual_returned_date, " &
                        "sr.quantity_requested AS quantity, sr.item_name AS item_name, NULL AS serial_number, " &
                        "'Not yet returned' AS accountability_status " &
                        "FROM supply_requests sr " &
                        "WHERE sr.user_id = @staffID AND sr.status IN ('Approved', 'approved', 'Released', 'released') " &
    ...
    Public Shared Function GetAllSuppliesRequests() As DataTable
        ...
        Try
            ...
            Dim query As String = "SELECT " &
                                 ...
                                 "IF(u.suffix IS NOT NULL AND u.suffix != '', CONCAT(' ', u.suffix), ''))) AS requester_name, " &
                                 "IFNULL(d.department_name, 'N/A') AS department, " &
                                 "sr.date_of_request AS date_of_request, " &
                                 "sr.item_name AS item_name, " &
                                 "sr.quantity_requested AS quantity_requested, " &
    ...
    Public Shared Function GetAllMaintenanceRequests() As DataTable
        ...
        Try
            ...
            Dim query As String = "SELECT mr.request_id, mr.date_requested, mr.item_name, mr.property_number, " &
                                 "mr.serial_number, mr.department_id, d.department_name, mr.location, " &
    ...
    Public Shared Function SubmitMaintenanceRequest(staffID As Integer, itemName As String, propertyNumber As String,
        ...
        Try
            ...
            Dim query As String = "INSERT INTO maintenance_requests (date_requested, item_name, property_number, " &
                                 "serial_number, department_id, location, condition_before, type_of_issue, " &
    ...
    Public Shared Function GetMaintenanceRecords(Optional statusFilter As String = "",
        ...
        Try
            ...
            query.Append("m.location, m.department_id, d.department_name, m.condition_before_maint, ")
            ...
            query.Append("LEFT JOIN departments d ON m.department_id = d.department_id WHERE 1=1 ")
    ...
    Public Shared Function LoadAdminProfile(adminID As String) As Dictionary(Of String, Object)
        ...
        Try
            ...
            Dim query As String = "SELECT user_id, first_name, middle_name, last_name, suffix, position, " &
                                 "department_id, contact_number, email, username, barangay, " &
                                 "municipal, province, employee_id, role, status, " &
    ...
    Public Shared Function GetAdminAccounts(Optional statusFilter As String = "",
        ...
        Try
            ...
            query.Append("department_id, contact_number, email, username, role AS user_type, status, ")
    ...
    Public Shared Function GetAllUsers(Optional statusFilter As String = "",
        ...
        Try
            ...
            ' Create unified schema for the result table
            dt.Columns.Add("user_id", GetType(Integer))
            dt.Columns.Add("first_name", GetType(String))
            dt.Columns.Add("middle_name", GetType(String))
            ...
            dt.Columns.Add("contact_number", GetType(String))
            dt.Columns.Add("email", GetType(String))
            dt.Columns.Add("username", GetType(String))
            dt.Columns.Add("user_type", GetType(String))
            dt.Columns.Add("status", GetType(String))
            dt.Columns.Add("employee_id", GetType(String))
            dt.Columns.Add("date_assigned", GetType(Object))
            dt.Columns.Add("last_login", GetType(Object))
            dt.Columns.Add("created_at", GetType(Object))
            ...
            If Not String.IsNullOrEmpty(searchKeyword) Then
                ...
                adminWhereConditions.Add("(LOWER(first_name) LIKE @search OR LOWER(last_name) LIKE @search OR " &
                                        ...
                                        "LOWER(COALESCE(employee_id, '')) LIKE @search)")
            ...
            adminQuery.Append("SELECT user_id, first_name, COALESCE(middle_name, '') as middle_name, last_name, ")
            adminQuery.Append("COALESCE(suffix, '') as suffix, COALESCE(position, '') as position, ")
            adminQuery.Append("COALESCE(department_id, '') as department_id, COALESCE(contact_number, '') as contact_number, ")
            adminQuery.Append("email, username, role AS user_type, status, ")
            adminQuery.Append("COALESCE(employee_id, '') as employee_id, date_assigned, last_login, created_at, ")
            ...
            staffQuery.Append("SELECT user_id, first_name, COALESCE(middle_name, '') as middle_name, last_name, ")
            staffQuery.Append("COALESCE(suffix, '') as suffix, COALESCE(position, 'Staff') as position, ")
            staffQuery.Append("COALESCE(CAST(department_id AS CHAR), '') as department_id, COALESCE(contact_number, '') as contact_number, ")
            staffQuery.Append("email, username, role AS user_type, status, ")
            staffQuery.Append("COALESCE(employee_id, '') as employee_id, date_assigned, last_login, created_at, ")
                ...
                Using cmd As New MySqlCommand(staffQuery.ToString(), conn)
                    ...
                    Using adapter As New MySqlDataAdapter(cmd)
                        ...
                        For Each record As DataRow In staffRecords.Rows
                            ...
                            row("first_name") = SafeDbValue(record("first_name"))
                            row("middle_name") = SafeDbValue(record("middle_name"))
                            row("last_name") = SafeDbValue(record("last_name"))
                            row("suffix") = SafeDbValue(record("suffix"))
                            row("position") = SafeDbValue(record("position"))
                            row("department_id") = SafeDbValue(record("department_id"))
                            row("contact_number") = SafeDbValue(record("contact_number"))
                            row("email") = SafeDbValue(record("email"))
                            row("username") = SafeDbValue(record("username"))
                            row("user_type") = "Staff"
                            row("status") = SafeDbValue(record("status"))
                            row("employee_id") = SafeDbValue(record("employee_id"))
                            row("date_assigned") = If(IsDBNull(record("date_assigned")) OrElse record("date_assigned") Is Nothing, DBNull.Value, record("date_assigned"))
                            row("last_login") = If(record.IsNull("last_login"), DBNull.Value, record("last_login"))
                            row("created_at") = If(record.IsNull("created_at"), DBNull.Value, record("created_at"))
        ...
        Catch ex As Exception
            System.Diagnostics.Debug.WriteLine("[v0] GetAllUsers Exception: " & ex.Message)
        Finally
            If conn IsNot Nothing Then
                Try
                    If conn.State = ConnectionState.Open Then conn.Close()
    ...
    Public Shared Function GetAdminContextByUsername(username As String) As Dictionary(Of String, String)
        ...
        Try
            ...
            Dim query As String = "SELECT user_id, username, role AS user_type FROM users WHERE LOWER(username) = LOWER(@username) LIMIT 1"
            Using cmd As New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@username", username.Trim())
                Using reader As MySqlDataReader = cmd.ExecuteReader()
                    If reader.Read() Then
    ...
    Public Shared Function UpdateAdminAccount(adminID As Integer,
        ...
        Try
            ...
            Dim updateQuery As String =
                ...
                "employee_id = @employeeID, role = @userType, status = @status, updated_at = NOW() " &
                "WHERE user_id = @adminID"
    ...
    Public Shared Function DeleteUserAccount(userID As Integer,
        ...
        Try
            ...
            If userType = "Admin" OrElse userType = "SuperAdmin" Then
                ...
                Using infoCmd As New MySqlCommand("SELECT department_id FROM users WHERE user_id = @userID LIMIT 1", conn)
            ...
            ElseIf userType = "Staff" Then
                ' Get department_id before deletion for headcount recalculation
                Using infoCmd As New MySqlCommand("SELECT department_id FROM users WHERE user_id = @userID AND role = 'Staff' LIMIT 1", conn)
                    infoCmd.Parameters.AddWithValue("@userID", userID)
                    Dim deptValue = infoCmd.ExecuteScalar()
                    If deptValue IsNot Nothing AndAlso deptValue IsNot DBNull.Value Then
                        affectedDepartmentID = Convert.ToInt32(deptValue)
                ...
                query = "DELETE FROM users WHERE user_id = @userID AND role = 'Staff'"
            Else
    ...
    Public Shared Function ResetUserPassword(userID As Integer,
        ...
        Try
            ...
            If userType = "Admin" OrElse userType = "SuperAdmin" OrElse userType = "Staff" OrElse userType = "Custodian" Then
                query = "UPDATE users SET password_encrypted = @password, updated_at = NOW() WHERE user_id = @userID"
            Else
                MessageBox.Show("Invalid user type specified.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return False
            End If
    ...
    Public Shared Function UpdateUserAccount(userID As Integer,
        ...
        Try
            ...
            If userType = "Admin" OrElse userType = "SuperAdmin" Then
                ...
                query = "UPDATE users SET first_name = @firstName, middle_name = @middleName, last_name = @lastName, " &
                       "suffix = @suffix, position = @position, department_id = @departmentID, contact_number = @contactNumber, " &
                       "email = @email, username = @username, house_no_street = @houseNo, barangay = @barangay, " &
                       "municipality = @municipality, province_city = @province, date_assigned = @dateAssigned, " &
                       "employee_id = @employeeID, role = @userType, status = @status, updated_at = NOW() " &
            ...
            ElseIf userType = "Staff" Then
                ...
                query = "UPDATE users SET first_name = @firstName, last_name = @lastName, " &
                       "email = @email, username = @username, contact_number = @contactNumber, " &
    ...
    Public Shared Function GetStaffAccounts(Optional statusFilter As String = "",
        ...
        Try
            ...
            Dim query As New StringBuilder()
            ' Query Staff accounts from users table (same table as Admin/SuperAdmin)
            query.Append("SELECT user_id as staff_id, first_name, last_name, email, contact_number, ")
            query.Append("department_id, username, COALESCE(position, 'Staff') as position, status, created_at ")
    ...
    Public Shared Function AddStaffAccount(firstName As String,
        ...
        Try
            ...
            Dim insertQuery As String =
                "INSERT INTO staff_accounts (first_name, last_name, email, contact_number, department_id, username, password_encrypted, position, status, created_at) " &
    ...
    Public Shared Function UpdateStaffAccount(staffID As Integer,
        ...
        Try
            ...
            Dim updateQuery As String =
                "UPDATE users SET first_name = @firstName, last_name = @lastName, email = @email, contact_number = @contactNumber, " &
                "department_id = @departmentID, username = @username, position = @position, status = @status, updated_at = NOW() " &
                "WHERE user_id = @staffID AND role = 'Staff'"
    ...
    Public Shared Function ResetStaffPassword(staffID As Integer,
        ...
        Try
            ...
            Using cmd As New MySqlCommand("UPDATE users SET password_encrypted = @password, updated_at = NOW() WHERE user_id = @staffID AND role = 'Staff'", conn)
                cmd.Parameters.AddWithValue("@password", hashedPassword)
                cmd.Parameters.AddWithValue("@staffID", staffID)
    ...
    Public Shared Function SetStaffStatus(staffID As Integer,
        ...
        Try
            ...
            Dim statusValue As String = If(isActive, "Active", "Inactive")
            Using cmd As New MySqlCommand("UPDATE users SET status = @status, updated_at = NOW() WHERE user_id = @staffID AND role = 'Staff'", conn)
                cmd.Parameters.AddWithValue("@status", statusValue)
                cmd.Parameters.AddWithValue("@staffID", staffID)
    ...
    Public Shared Function DeleteStaffAccount(staffID As Integer,
        ...
        Try
            ...
            Using cmd As New MySqlCommand("DELETE FROM users WHERE user_id = @staffID AND role = 'Staff'", conn)
                cmd.Parameters.AddWithValue("@staffID", staffID)
    ...
    Public Shared Function GetCustodians(Optional includeInactive As Boolean = True,
        ...
        Try
            ...
            Dim query As New StringBuilder()
            query.Append("SELECT user_id, first_name, last_name, position, contact_number, email, username, status, last_login, date_assigned  ")
    ...
    Public Shared Function GetAllProperties(Optional custodianID As Integer? = Nothing, Optional conditionStatus As String = "",
        ...
        Try
            ...
            ' Build query with optional filters - includes all required fields including description
            Dim query As String = "SELECT p.property_id, p.item_name, p.category, p.property_number, p.serial_number, " &
                                 "p.description, p.condition, p.acquisition_cost, p.acquisition_date, " &
                                 "p.assigned_to, CONCAT(IFNULL(u.first_name,''), ' ', IFNULL(u.last_name,'')) AS assigned_employee, " &
            ...
            If custodianID.HasValue Then
                query &= " AND p.assigned_to = @custodianID"
            End If
    ...
    Public Shared Function UpdateProperty(propertyID As Integer, propertyName As String, category As String,
        ...
        Try
            ...
            Dim query As String = "UPDATE properties SET item_name = @propertyName, category = @category, " &
                                 "description = @description, serial_number = @serialNumber, condition = @conditionStatus, " &
                                 "location = @location, assigned_to = @custodianID, department_id = @departmentID, " &
                                 "acquisition_date = @acquisitionDate, acquisition_cost = @acquisitionCost, " &
    ...
    Public Shared Function DeleteProperty(propertyID As Integer) As Boolean
        ...
        Try
            ...
            Dim query As String = "UPDATE properties SET status = 'For Disposal', updated_at = NOW() WHERE property_id = @propertyID"
            Using cmd As New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@propertyID", propertyID)
    ...
    Public Shared Function GetPropertyLifecycleInfo(propertyID As Integer) As Dictionary(Of String, Object)
        ...
        Try
            ...
            Using cmd As New MySqlCommand(query, conn)
                ...
                Using reader As MySqlDataReader = cmd.ExecuteReader()
                    If reader.Read() Then
                        result("item_name") = reader("item_name").ToString()
    ...
    Public Shared Function GetAllSupplies(Optional category As String = "", Optional status As String = "") As DataTable
        ...
        Try
            ...
            Dim query As String = "SELECT " &
                                 ...
                                 "item_name AS SupplyName, " &
                                 "category AS Category, " &
                                 "description AS Description, " &
                                 "unit_of_measure AS UnitOfMeasure, " &
                                 "quantity AS QuantityInStock, " &
                                 "unit_cost AS UnitCost, " &
                                 "total_cost AS TotalValue, " &
                                 "date_received AS AcquisitionDate, " &
    ...
    Public Shared Function GetSupplyById(supplyID As Integer) As DataRow
        ...
        Try
            ...
            Dim query As String = "SELECT supply_id, item_name, category, description, unit_of_measure, quantity, " &
                                  "supplier, unit_cost, location, stock_status, date_received, total_cost, source_of_funds " &
    ...
    Public Shared Function GetSupplyStockLevels(Optional includeLowStockOnly As Boolean = False,
        ...
        Try
            ...
            Dim query As New StringBuilder()
            query.Append("SELECT supply_id, item_name, category, quantity, unit_cost, ")
            query.Append("total_cost, stock_status, location FROM supplies WHERE 1=1 ")
    ...
    Public Shared Function UpdateSupply(supplyID As String, supplyName As String, category As String,
        ...
        Try
            ...
            Dim query As String = "UPDATE supplies SET item_name = @supplyName, category = @category, " &
                                 "quantity = @stock, unit_cost = @unitCost, total_cost = @totalValue, " &
                                 "stock_status = @status, location = @location, description = @description, " &
                                 "supplier = @supplierName, updated_at = NOW() " &
    ...
    Public Shared Sub RecalculateDepartmentHeadcount(Optional departmentID As Integer? = Nothing)
        ...
        Try
            ...
            For Each deptId As Integer In targets
                ...
                Using staffCmd As New MySqlCommand("SELECT COUNT(*) FROM users WHERE department_id = @dept AND role = 'Staff' AND status = 'active'", conn)
    ...
    Public Shared Function AddDepartment(departmentName As String, headOfDepartment As String, location As String,
        ...
        Try
            ...
            Dim query As String = "INSERT INTO departments (department_name, head_of_department, location, office_code, " &
    ...
    Public Shared Function UpdateDepartment(departmentID As Integer, departmentName As String, headOfDepartment As String,
        ...
        Try
            ...
            Dim query As String = "UPDATE departments SET department_name = @departmentName, head_of_department = @headOfDepartment, " &
                                 "location = @location, office_code = @departmentCode, contact_number = @contactNumber, " &
    ...
    Public Shared Function DeleteDepartment(departmentID As Integer) As Boolean
        ...
        Try
            ...
            Dim checkPropertiesQuery As String = "SELECT COUNT(*) FROM properties WHERE department_id = @departmentID AND status != 'For Disposal' AND status != 'Lost'"
            ...
            Dim checkStaffQuery As String = "SELECT COUNT(*) FROM users WHERE department_id = @departmentID AND role = 'Staff' AND status = 'active'"
    ...
    Public Shared Function GetDepartmentStats(departmentID As Integer) As Dictionary(Of String, Integer)
        ...
        Try
            ...
            ' Get employee count
            Dim employeeQuery As String = "SELECT COUNT(*) FROM users WHERE department_id = @departmentID AND role = 'Staff' AND status = 'active'"
            Using cmd As New MySqlCommand(employeeQuery, conn)
                cmd.Parameters.AddWithValue("@departmentID", departmentID)
    ...
    Public Shared Sub InitializeDefaultAccounts()
        ...
        Try
            ...
            Using checkCmd As New MySqlCommand("SELECT user_id FROM users WHERE LOWER(username) = LOWER(@username) AND role = 'SuperAdmin' LIMIT 1", conn)
            ...
            Using checkCmd As New MySqlCommand("SELECT user_id FROM users WHERE LOWER(username) = LOWER(@username) AND role = 'Admin' LIMIT 1", conn)
            ...
            Else
                ' Update existing Custodian account to ensure credentials stay in sync
                Using updateCmd As New MySqlCommand("UPDATE users SET first_name = @firstName, last_name = @lastName, email = @email, " &
                                                    "password_encrypted = @password, status = 'Active', updated_at = NOW() WHERE user_id = @userID AND role = 'Custodian'", conn)
            ...
            Using checkCmd As New MySqlCommand("SELECT user_id FROM users WHERE LOWER(username) = LOWER(@username) AND role = 'Staff' LIMIT 1", conn)
            ...
            If staffId Is Nothing OrElse staffId Is DBNull.Value Then
                ...
                Using insertCmd As New MySqlCommand("INSERT INTO users (first_name, last_name, email, username, password_encrypted, role, status, position, created_at) " &
            ...
            Else
                ...
                Using updateCmd As New MySqlCommand("UPDATE users SET first_name = @firstName, last_name = @lastName, email = @email, " &
                                                    "password_encrypted = @password, status = 'Active' WHERE user_id = @userID AND role = 'Staff'", conn)
